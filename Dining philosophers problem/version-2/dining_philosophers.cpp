 ...
 * This is a deadlock-free solution, as each philosopher picks up both chopsticks near it in a critical section guarded by a global mutex.
 * @author: ArvindRS
 * @date: 09/05/2017
 */
	...
	while(args->term_signal == 0) {
		...
		if(state == 1) {
			...
			while(hungry) {
				...
				if(args->chopsticks[number] && !args->chopsticks[(number+1)%N])
